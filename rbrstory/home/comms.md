...
## The device network ##
The relay modules run ESP-Now, a lightweight networking protocol that runs independently of the house HTTP network. One of the devices is designated the "master" or "hub" device; this connects to the house router and from there to the system controller. All the other devices are designated "slaves"; these connect to the master using ESP-Now. The outcome of this is that the system can be preconfigured, using the router at the supplier's location, then on delivery the system controller and the master device are switched over to the house network. We have developed a configuration program, that runs on any Linux computer to perform this task quickly and easily.
